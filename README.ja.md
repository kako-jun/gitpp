...
- **完了 bloom** — 完了したリポジトリ名を状態色へ短くフェード表示。再実行でも再び動作し、Unicode名でも端末幅を崩しません
...
```
┌──────────────────────────────────────────────────────────────┐
│ gitpp  j/k:move  Enter:detail  g/G:top/end  n/N:next issue │
...
| `j` / `k` / `↑` / `↓` | リポジトリを選択・スクロール |
| `g` / `G` | 先頭 / 末尾へ移動 |
| `n` / `N` | 次 / 前の要対応リポ（Failed または Blocked）にジャンプ |
| `Enter` | 詳細ペインの表示/非表示 |
