...
| diff | `git diff --stat` | Read-only; no config applied |
| fetch | `git fetch` | Read-only; no config applied |
| branch | `git rev-parse --abbrev-ref HEAD` | Read-only; no config applied |
| switch | `git rev-parse --verify main` → `git switch main` (or master) | Detects default branch automatically |
...
- **diff**: `had_changes` is true when there are unstaged differences (non-empty output)
- **fetch**: `had_changes` is always false (fetch does not modify the working tree)
